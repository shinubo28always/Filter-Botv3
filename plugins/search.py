        ...
        for f in fsub_channels:
            try:
                ...
                if status in ['left', 'kicked']:
                    ...
                    btn = "✨ ʀᴇǫᴜᴇsᴛ ᴛᴏ ᴊᴏɪɴ ✨" if is_req else "✨ ᴊᴏɪɴ ᴄʜᴀɴɴᴇʟ ✨"
                    markup = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton(btn, url=invite.invite_link))
...
def handle_fuz_click(call):
    ...
    # SECURITY: User-Lock with Popup
    if clicker_id != int(ouid) and not db.is_admin(clicker_id):
        return bot.answer_callback_query(call.id, "⚠️ Ye aapka request nahi hai! Apna khud ka search karein.", show_alert=True)
    
    data = db.get_filter(key)
    if not data:
        # Fallback if key was short
        matches = process.extractOne(key, db.get_all_keywords())
        if matches: data = db.get_filter(matches[0])
...
def send_final_result(message, data, r_mid):
    ...
    try:
        # NO EFFECTS HERE (CRASH PREVENTION)
        bot.copy_message(
            chat_id=target_chat,
        ...
        bot.send_message(target_chat, "❌ <b>Post missing!</b>\nYeh anime database channel se delete ho gaya hai.", reply_to_message_id=r_mid)
