...
def handle_callbacks(call):
    ...
    if data[0] in ["ind", "res"]:
        ...
        if call.from_user.id != searcher_id and not db.is_admin(call.from_user.id):
            return bot.answer_callback_query(call.id, "⚠️ This is not your result!", show_alert=True)
...
def send_index_page(chat_id, letter, page, original_mid, uid, edit_mid=None):
    ...
    nav = [types.InlineKeyboardButton("⬅️ Back", callback_data=f"ind|{letter}|{page-1}|{uid}|{original_mid}") if page > 1 else None,
           types.InlineKeyboardButton(f"{page}/{total_pages}", callback_data="none"),
           types.InlineKeyboardButton("Next ➡️", callback_data=f"ind|{letter}|{page+1}|{uid}|{original_mid}") if page < total_pages else None]
    markup.row(*[b for b in nav if b])
    bot.edit_message_text(f"📂 <b>Anime Index: '{letter.upper()}'</b>\nTotal Results: <code>{len(results)}</code>", chat_id, edit_mid, reply_markup=markup)
